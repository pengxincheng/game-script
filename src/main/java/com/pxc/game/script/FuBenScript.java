...
public class FuBenScript {
    ...
    public static void start(String fubenType,Boolean left) throws Exception {
        ...
        while (true){
            ...
            if(zhdou.contains("普通")&& (zhdou.contains("已完成") )){
                logger.info("当前副本已经完成");
                OperationUtils.findAndClick("已完成", donghuaPaht);
                TimeUnit.SECONDS.sleep(3);
                break;

            }else if (zhdou.contains("普通")) {
            ...
            else if (zhdou.contains("绿梦如烟")) {

                OperationUtils.findAndClick("绿梦如烟", zhandouPath);
                TimeUnit.SECONDS.sleep(5);
            ...
            if (zhdou.contains("侠士") && (zhdou.contains("已完成"))) {
                logger.info("当前副本已经完成");
                OperationUtils.findAndClick("已完成", donghuaPaht);
                TimeUnit.SECONDS.sleep(3);
                break;

            } else if (zhdou.contains("侠士")) {
    ...
    private static void click(String word,String path)throws Exception{
        ...
        if(word.contains("跳过剧情")){
            OperationUtils.findAndClick("跳过剧情",path);
            TimeUnit.SECONDS.sleep(2);
        }

        if(word.contains("手动")){
            OperationUtils.findAndClick("手动",path);
            TimeUnit.SECONDS.sleep(2);
        }
...
//                }
        ...
        if(word.contains("绿梦")){
            OperationUtils.findAndClick("绿梦",path);
            TimeUnit.SECONDS.sleep(2);
        }
